...
func (service Service) GetProductDetail(params dto.ScrapperParams) *dto.RetrievedProduct {
	...
	err := service.svc.InitAndRunActions(params.Url, []chromedp.Action{
		...
		chromedp.ActionFunc(service.svc.MouseActions(".vtex-store-components-3-x-productNameContainer .vtex-store-components-3-x-productBrand")),
		// Esperar la carga
		...
		//
		chromedp.Text(`.valtech-carrefourar-product-price-0-x-currencyContainer`, &product.OriginalDiscountPrice, chromedp.NodeVisible),
		chromedp.Text(`.vtex-store-components-3-x-productNameContainer .vtex-store-components-3-x-productBrand`, &product.Name, chromedp.ByQuery),
		chromedp.Text(`.vtex-store-components-3-x-productBrandName`, &product.Brand, chromedp.ByQuery),
