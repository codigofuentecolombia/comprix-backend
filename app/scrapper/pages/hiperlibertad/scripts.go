...
func (service *Service) GetPageProductLinksScript() string {
	return `
        return Array.from(document.querySelectorAll('#gallery-layout-container > div a')).map( el => el.href ).slice(0, 48);
    `
}
...
func (service *Service) GetImagesScript() string {
	return `
		return Array.from(document.querySelectorAll('.vtex-store-components-3-x-carouselGaleryCursor .vtex-store-components-3-x-productImage img')).map(img => img.src);
    `
}

func (service *Service) GetOriginalPriceScript() string {
	return `
        let priceElement = document.querySelector(".vtex-product-price-1-x-listPriceValue > span");
            ...
            if( document.querySelector("#gallery-layout-container") ){
                ...
                while( !areAllProductsReady && (maxTries > currentTries) ) {
                    ...
                    if( currentTotalProducts == totalProducts ){
                        ...
                        let reloadElement1 = document.querySelector(".vtex-search-result-3-x-buttonShowMore--layout")
                        // Validar si existe
                        if(reloadElement1){
...
func (service *Service) WaitUntilProductPriceIsLoaded() string {
	return `
        ...
        function checkPrice() {
            ...
            let priceElement = document.querySelector(".vtex-product-price-1-x-sellingPrice--pdp-selling-price");

            if (priceElement) {
                let priceText = priceElement.textContent.trim().replaceAll(" ","");
            ...
            if (attemptCount < maxAttempts) {
                attemptCount++;
                setTimeout(checkPrice, 3000); // Vuelve a intentar después de 1.5 segundos
            } else {
                // Si el precio es válido, crea el nuevo elemento con ID 'succes-load'
                let successElement = document.createElement('div');
                successElement.id = 'succes-load';
