...
func (service Service) GetProductDetail(params dto.ScrapperParams) *dto.RetrievedProduct {
	...
	err := service.svc.InitAndRunActions(params.Url, []chromedp.Action{
		...
		chromedp.ActionFunc(service.svc.MouseActions(".vtex-store-components-3-x-productBrand--pdp-product-name")),
		// Esperar la carga
		...
		chromedp.Text(`.vtex-store-components-3-x-productBrand--pdp-product-name`, &product.Name, chromedp.NodeVisible),
		chromedp.Text(`.vtex-product-price-1-x-sellingPrice--pdp-selling-price`, &product.OriginalDiscountPrice, chromedp.NodeVisible),
		chromedp.Text(`.vtex-store-components-3-x-productDescriptionText`, &product.Description, chromedp.NodeVisible),
		chromedp.Text(`.vtex-store-components-3-x-productBrandName`, &product.Brand, chromedp.NodeVisible),
		//
		chromedp.AttributeValue(`meta[property="product:retailer_part_no"]`, "content", &product.Sku, nil),
		// Obtener product
