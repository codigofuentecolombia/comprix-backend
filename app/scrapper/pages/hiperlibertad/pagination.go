...
func (service Service) GetTotalPages(params dto.ScrapperParams) int {
	...
	err := service.svc.InitAndRunActions(params.Url, []chromedp.Action{
		...
		chromedp.WaitVisible(".vtex-search-result-3-x-totalProducts--layout > span", chromedp.ByQuery),
		chromedp.Text(`.vtex-search-result-3-x-totalProducts--layout > span`, &totalProducts, chromedp.NodeVisible),
	}, 80)
...
func (service Service) GetProductLinksByPage(params dto.ScrapperParams) []string {
	...
	err := service.svc.InitAndRunActions(currentPageUrl, []chromedp.Action{
		...
		chromedp.WaitVisible(`.vtex-search-result-3-x-totalProducts--layout`, chromedp.ByQuery),
